...
**F — Phase 1B, the big one (ADR-0017 + §7).** Canonical Claude `SessionBundle`: logical identity `{source, projectSlug, sessionId}`, opaque versioned refs (Claude only — deliberate two-scheme asymmetry), a shared resolver for `list`/`detail`/`collectEvents`/search, one card per identity, and a source-agnostic versioned `ccv.starred` migration. Then the subagent/history/folder-only/index-only converter with the `{meta, events, resolvedOpts}` contract. Two standing constraints: the `claude.js` `list()` refactor must preserve the context-health computation now living there, and no 1B parsing work starts before the resolver/identity contract (already accepted as ADR-0017) is implemented first.
...
- Current baseline: `main` at `c40d385` (items D+E), tracked tree clean, `npm test` 114/0 (verified 2026-07-07).
...
If you want to start now, item F is all that remains — begin with the ADR-0017 resolver/identity contract loop (no 1B parsing before it lands).
